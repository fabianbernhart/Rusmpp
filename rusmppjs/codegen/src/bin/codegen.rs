...
use serde_reflection::{Registry, Result, Tracer, TracerConfig};
use std::{fs, io::Write};
...
fn main() -> Result<(), Box<dyn error::Error>> {
    ...
    // Tell the tracer which types from my_crate to track
    ...
    let registry: Registry = tracer.registry()?;
    ...
    let mut file = std::fs::File::create("./src/_generated.rs")?;
